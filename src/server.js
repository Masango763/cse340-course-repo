...
const initializeDatabase = async () => {
  try {
    ...
    // Check if projects exist, if not or if we want IT projects, insert them
    const checkProjects = await pool.query('SELECT COUNT(*) FROM projects');
    if (parseInt(checkProjects.rows[0].count) === 0) {
      ...
      `);
      console.log('Successfully seeded IT-focused service projects!');
    }
  } catch (err) {
